...
### 7. Write Proposal Flow State Machine

**Location**: `org.apache.jackrabbit.oak.segment.http.wallet.SlingWriteProposalService`

Client-side flow for proposing signed write transactions.

```
                    ┌─────────────────────────────────────────┐
                    │      WRITE PROPOSAL FLOW (CLIENT)       │
                    └─────────────────────────────────────────┘
                              ...
                              └─────┬─────┘
                                    │
                                    │ proposeWrite(contentType, message)
                                    ▼
                              ┌───────────┐
                              │  CHECK    │
                              │  WALLET   │
                    ...
                    ┌───────────────┴───────────────┐
                    │                               │
                    │ Wallet unavailable            │ Wallet available
                    │                               │
                    ▼                               ▼
              ┌───────────┐                   ┌───────────┐
              │  FAILURE  │                   │  CREATE   │
              ...
              └───────────┘                   │  MESSAGE  │
                                              └─────┬─────┘
                                                    │
                                                    │ Format: wallet:timestamp:contentType:message
                                                    ▼
                                              ┌───────────┐
                                              │   SIGN    │
                                              │ TRANSACTION│
...
│                                                                            │
│ Example: 0x742d35Cc6634C0532925a3b844Bc9e7595f8fEb:1704844800:page:Hello  │
└────────────────────────────────────────────────────────────────────────────┘
```

---

### 8. Delete Proposal Flow State Machine

**Location**: `org.apache.jackrabbit.oak.segment.http.wallet.SlingDeleteProposalService`

Client-side flow for proposing signed delete transactions with ownership verification.

```
                    ┌─────────────────────────────────────────┐
                    │     DELETE PROPOSAL FLOW (CLIENT)       │
                    └─────────────────────────────────────────┘
              ...
              └───────────┘                   └─────┬─────┘
                                    ...
                                    ┌───────────────┴───────────────┐
                                    │                               │
                                    │ Path not under                │ Path under
                                    │ /oak-chain/content/{wallet}/  │ /oak-chain/content/{wallet}/
                                    │                               │
                                    ▼                               ▼
                              ┌───────────┐                   ┌───────────┐
                              │ OWNERSHIP │                   │  CREATE   │
                              ...
                              └───────────┘                   │  MESSAGE  │
                                                              └─────┬─────┘
                                                                    │
                                                                    │ Format: wallet:timestamp:delete:path
                                                                    ▼
                                                              ┌───────────┐
                                                              │   SIGN    │
                                                              │ TRANSACTION│
...
┌────────────────────────────────────────────────────────────────────────────┐
│ Content path MUST start with: /oak-chain/content/{walletAddress}/          │
│                                                                            │
│ Valid:   /oak-chain/content/0x742d35.../page-123                          │
│ Invalid: /oak-chain/content/0xABCDEF.../page-456 (different wallet)       │
└────────────────────────────────────────────────────────────────────────────┘
```
...
| **oak-segment-http** | 3 | HttpPersistenceService lifecycle, Write flow, Delete flow |
